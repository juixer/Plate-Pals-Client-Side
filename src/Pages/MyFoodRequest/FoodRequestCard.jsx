...
const FoodRequestCard = ({ req }) => {
  ...
  const handleCancel = (id) => {
    ...
    const cancel =  {
        money, food_status, request_date, requester_email, requester_image, requester_name
    }
    axios
    .patch(`http://localhost:5000/api/request/${id}`, cancel)
    .then((res) => {
    ...
    <Card className="max-w-sm">
      ...
      <div className="flex flex-col items-center pb-10">
        ...
        <div className="mt-4 flex justify-center items-center flex-col space-x-3 lg:mt-6 space-y-1">
          ...
          {food_status === 'available' || food_status === 'pending' ? <button onClick={()=>handleCancel(_id)} className="px-3 py-2 text-white bg-red-500 rounded-lg">Cancel</button> : ''}
        </div>
      </div>
    </Card>
  );
